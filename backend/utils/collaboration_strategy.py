# backend/utils/collaboration_strategy.py
import re
import random
from typing import Dict, List, Optional
from backend.utils.base import get_typeName
...
def needs_tool(message: str, window: int = 8) -> bool:
    ...
    window: 否定词检测窗口（字符数），否定词出现在关键词前 N 个字符内才生效
    """
    msg = message.lower()
    ...
    for indicator in tool_indicators:
        ...
        if idx == -1:
            continue  # 没命中这个关键词，继续下一个
        
        # 2. 检查关键词前面 window 个字符内是否有否定词
...
async def select_model_by_strategy(
    ...
    if strategy == "hybrid":
        ...
        ratio = getattr(collab_config, 'primary_ratio', getattr(collab_config, 'primary_ratio', 70))
        if roll <= ratio:
