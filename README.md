...
LumNeo 是一款跨平台 AI 桌面应用，将**本地隐私**与**云端算力**融为一体。它不只是对话框，更是可自由塑造的**智能体工作台**——支持多角色切换、文件读写解析、MCP 工具扩展，并通过 MCP 协议预留了完整的硬件设备接入能力，让 AI 不仅读懂你的文档，更能感知物理世界。
...
### 硬件设备接入，天生就绪
- **MCP 协议原生支持硬件通信**：串口、USB、蓝牙设备均可通过 MCP Server 接入，无需修改核心代码
- **Python 后端生态天然适配**：pyserial、pyusb、esptool 等硬件工具库可直接在后台调用
- **桌面端原生权限**：基于 PyWebView 的桌面容器，可直连系统硬件接口，不受浏览器沙箱限制
- **为 AIoT 场景预留**：当前版本已具备完整的硬件工具调用链路，为 Phase 3 的 IoT 控制铺平道路
...
| **Phase 3** | **Future** | **打通物理世界**<br> 基于 MCP 协议提供官方硬件接入模板（串口通信、GPIO 控制、传感器数据读取），支持 Renode 等模拟器集成与固件烧录管理，让 AI 助手真正“动手”控制硬件设备。 |  规划中 |
...
欢迎提 Issue、Pull Request，或分享你的角色配置、MCP 工具和 Skill。**特别欢迎硬件方向的贡献者**——如果你开发了串口调试、传感器数据读取、固件烧录等硬件 MCP 工具，欢迎提交到社区，一起打造 **AI 原生硬件开发工作台**。  LumNeo 因你而更温暖，每一行代码都是点亮灵感的光 
