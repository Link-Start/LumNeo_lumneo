...
from backend.memory.capture import capture
from backend.memory.model import ConversationTurn, MemoryCandidate, Source, Evidence
from backend.memory.common.time import utc_now
from backend.memory.capture.provider import CaptureProvider, CaptureError
...
def test_dedup_key_auto_completion():
    ...
    assert result[0].dedup_key is not None
    from backend.memory.common.hash_utils import compute_dedup_key
