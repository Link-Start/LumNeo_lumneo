...
#   - ToolPersister（封装 ToolCallRepository Port + StoragePort）
#   - ApprovalHandler
#   - SuggestionGenerator
# 完成上述职责，自身不直接 import 数据库或文件 I/O 模块（§60）。
import asyncio
import json
import time
...
from lumneo.runtime.tools.registry import execute_tool
...
class ToolExecutor:
    ...
    async def _execute_with_retry(
        ...
        """执行单个工具，支持重试。返回：(result, failed, exec_time_ms, error_message)。"""
